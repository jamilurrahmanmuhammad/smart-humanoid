...
- [x] T001 [B] Initialize Python project structure with pyproject.toml
  - Files: `pyproject.toml`, `backend/__init__.py`
  - Acceptance: `pip install -e .` succeeds

- [x] T002 [B] Create requirements.txt with pinned dependencies
...
- [x] T003 [B] Create .env.example with all required environment variables
  - Files: `.env.example`
  - Variables: OPENAI_API_KEY, QDRANT_URL, QDRANT_API_KEY, QDRANT_COLLECTION_NAME, DATABASE_URL, APP_ENV, LOG_LEVEL, CORS_ORIGINS

- [x] T003a [RED] [B] Write test for connectivity sanity-check utility
...
- [x] T003b [GREEN] [B] Implement connectivity sanity-check script
...
- [x] T003c [B] Implement feature readiness gate
...
- [x] T004 [B] Set up pytest configuration with asyncio mode
  - Files: `pytest.ini`, `conftest.py`
  - Acceptance: `pytest --collect-only` shows test collection

- [x] T005 [B] Create backend directory structure per plan.md
...
- [x] T006 [RED] [B] Write test for Settings model validation
  - File: `tests/unit/test_config.py`
  - Test: Settings loads from env, validates required fields, provides defaults
  - FR: N/A (infrastructure)

- [x] T007 [GREEN] [B] Implement Settings with pydantic-settings
...
- [x] T008 [RED] [B] Write test for ChatSession ORM model
...
- [x] T016 [RED] [B] Write test for async database session factory
  - File: `tests/unit/test_database.py`
  - Test: Creates async session, handles connection errors
  - FR: N/A (infrastructure)

- [x] T017 [GREEN] [B] Implement async session factory
...
- [x] T018 [B] Initialize Alembic with async support
  - Files: `alembic.ini`, `backend/alembic/env.py`
  - Config: async_fallback=True, target_metadata from models

- [x] T019 [B] Create initial migration for all models
...
- [x] T036 [RED] [B] [US1] Write test for embedding generation
  - File: `tests/unit/test_embedding.py`
  - Test: Generates 1536-dim vector, handles empty input
  - FR: N/A (infrastructure)

- [x] T037 [GREEN] [B] [US1] Implement EmbeddingService
...
- [x] T045c [RED] [B] [US1] Write test for context window summarization
  - File: `tests/unit/test_rag_pipeline.py`
  - Test: When context exceeds limit, earlier turns are summarized (not truncated)
  - FR: FR-013

- [x] T045d [GREEN] [B] [US1] Implement context summarization
  - File: `backend/services/rag.py`
...
- [x] T045e [RED] [P] [US1] Write test for cross-chapter context stitching
  - File: `tests/unit/test_rag_pipeline.py`
  - Test: Query spanning multiple chapters produces coherent combined context
  - FR: FR-014

- [x] T045f [GREEN] [P] [US1] Implement cross-chapter stitching
  - File: `backend/services/rag.py`
...
- [x] T050 [RED] [P] [US1] Write test for health check endpoint
  - File: `tests/integration/test_health.py`
  - Test: Returns healthy/degraded/unhealthy, checks all dependencies
  - FR: N/A (operations)

- [x] T051 [GREEN] [P] [US1] Implement GET /health endpoint
...
- [x] T052 [RED] [B] [US1] Write test for POST /chat endpoint
...
- [x] T053 [GREEN] [B] [US1] Implement POST /chat endpoint
...
- [x] T054 [RED] [B] [US1] Write test for POST /chat/stream endpoint
  - File: `tests/integration/test_chat_api.py`
  - Test: Returns SSE stream with content/citation/done events
  - FR: NFR-001

- [x] T055 [GREEN] [B] [US1] Implement POST /chat/stream endpoint
  - File: `backend/api/routes/chat.py`
...
- [x] T056 [RED] [B] [US1] Write test for WebSocket connection lifecycle
  - File: `tests/integration/test_websocket.py`
  - Test: Connect, receive welcome, send message, receive stream, disconnect
  - FR: FR-003

- [x] T057 [GREEN] [B] [US1] Implement WebSocket /ws/chat/{session_id}
  - File: `backend/api/routes/chat.py`
  - Protocol per contracts/websocket.md
  - Acceptance: T056 passes

- [x] T058 [RED] [P] [US1] Write test for WebSocket error handling
  - File: `tests/integration/test_websocket.py`
  - Test: Invalid message format, rate limiting, reconnection
  - FR: FR-028

- [x] T059 [GREEN] [P] [US1] Implement WebSocket error handling
  - File: `backend/api/routes/chat.py`
...
- [x] T060 [B] [US1] Create FastAPI application entry point
...
- [x] T061 [RED] [P] [US1] Write contract test for OpenAPI schema compliance
  - File: `tests/contract/test_openapi_schema.py`
  - Test: Responses match contracts/openapi.yaml schemas
  - FR: N/A (contract validation)

- [x] T062 [GREEN] [P] [US1] Fix any contract violations
  - Files: Various based on test failures
  - Fixed: Citation.relevance_score made optional per OpenAPI spec
...
- [x] T063 [RED] [B] [US2] Write test for persona system prompt injection
...
- [x] T067 [RED] [B] [US2] Write test for Agent with persona context
  - File: `tests/unit/test_agent.py`
  - Test: Agent uses persona system prompt
  - FR: FR-015

- [x] T068 [GREEN] [B] [US2] Integrate PersonaAdapter into AgentRunner
  - File: `backend/services/agent.py`
...
- [x] T069 [RED] [P] [US2] Write test for session persona storage
  - File: `tests/integration/test_sessions.py`
  - Test: Session stores persona, subsequent messages use same persona
  - FR: FR-012

- [x] T070 [GREEN] [P] [US2] Implement session persona persistence
...
- [x] T071 [RED] [B] [US5] Write test for safety keyword detection
  - File: `tests/unit/test_safety_guardrails.py`
  - Test: Detects motor, gripper, actuator, power, voltage keywords
  - FR: FR-021

- [x] T072 [GREEN] [B] [US5] Implement SafetyChecker keyword detection
...
- [x] T073 [RED] [B] [US5] Write test for disclaimer injection
  - File: `tests/unit/test_safety_guardrails.py`
  - Test: Disclaimer text prepended when safety keywords detected
  - FR: FR-022, FR-023

- [x] T074 [GREEN] [B] [US5] Implement disclaimer injection
  - File: `backend/services/safety.py`
...
- [x] T075 [RED] [P] [US5] Write test for safety category classification
  - File: `tests/unit/test_safety_guardrails.py`
  - Test: Classifies into motor_control, power_systems, physical_assembly
  - FR: FR-024

- [x] T076 [GREEN] [P] [US5] Implement safety category classification
  - File: `backend/services/safety.py`
...
- [x] T077 [RED] [B] [US5] Write test for safety check in RAG pipeline
  - File: `tests/unit/test_rag_pipeline.py`
  - Test: RAG response includes has_safety_disclaimer flag
  - FR: FR-021

- [x] T078 [GREEN] [B] [US5] Integrate SafetyChecker into RAGPipeline
  - File: `backend/services/rag.py`
...
- [x] T079 [RED] [B] [US3] Write test for chapter-filtered vector search
  - File: `tests/unit/test_vector_store.py`
  - Test: Search with chapter_id filter returns only matching chunks
  - FR: FR-003

- [x] T080 [GREEN] [B] [US3] Implement chapter filter in VectorStoreClient
  - File: `backend/services/vector_store.py`
...
- [x] T081 [RED] [B] [US3] Write test for page query type routing
  - File: `tests/unit/test_rag_pipeline.py`
  - Test: query_type=page applies chapter filter
  - FR: FR-003

- [x] T082 [GREEN] [B] [US3] Implement page query type in RAGPipeline
  - File: `backend/services/rag.py`
...
- [x] T083 [RED] [P] [US3] Write test for WebSocket context update
  - File: `tests/integration/test_websocket.py`
  - Test: Context message updates current_chapter
  - FR: FR-012

- [x] T084 [GREEN] [P] [US3] Implement context update handler
  - File: `backend/api/routes/chat.py`
...
- [x] T085 [RED] [B] [US4] Write test for selection text validation
  - File: `tests/unit/test_schemas.py`
  - Test: Rejects selection > 2000 chars, accepts valid selection
  - FR: FR-018

- [x] T086 [GREEN] [B] [US4] Implement selection validation in ChatRequest
  - File: `backend/models/schemas.py`
...
- [x] T087 [RED] [B] [US4] Write test for selection-only context
  - File: `tests/unit/test_rag_pipeline.py`
  - Test: query_type=selection uses only selected_text as context
  - FR: FR-018, FR-019

- [x] T088 [GREEN] [B] [US4] Implement selection query in RAGPipeline
  - File: `backend/services/rag.py`
  - Modification: Skip vector search for selection queries
  - Acceptance: T087 passes

- [x] T088a [RED] [P] [US4] Write test for insufficient selection response
  - File: `tests/unit/test_rag_pipeline.py`
  - Test: Selection-scoped query with insufficient context returns explicit message
  - FR: FR-020

- [x] T088b [GREEN] [P] [US4] Implement insufficient selection handling
  - File: `backend/services/rag.py`
...
- [x] T089 [RED] [P] [US4] Write test for is_selection_scoped flag
  - File: `tests/integration/test_chat_api.py`
  - Test: Response includes is_selection_scoped=true for selection queries
  - FR: FR-019

- [x] T090 [GREEN] [P] [US4] Implement is_selection_scoped in response
  - File: `backend/api/routes/chat.py`
...
- [x] T093 [RED] [P] [US6] Write test for chat message analytics
  - File: `tests/integration/test_analytics.py`
  - Test: Chat request logs query_received, response_sent events
  - FR: FR-029

- [x] T094 [GREEN] [P] [US6] Integrate analytics into chat endpoints
  - File: `backend/api/routes/chat.py`
...
- [x] T095 [RED] [P] [US6] Write test for latency tracking
  - File: `tests/unit/test_analytics.py`
  - Test: Tracks first_token_ms, total_ms in analytics
  - FR: NFR-001

- [x] T096 [GREEN] [P] [US6] Implement latency tracking
  - File: `backend/services/analytics.py`
...
- [x] T097 [RED] [B] Write test for POST /sessions
  - File: `tests/integration/test_sessions.py`
  - Test: Creates session with persona, returns SessionResponse
  - FR: FR-012

- [x] T098 [GREEN] [B] Implement POST /sessions endpoint
  - File: `backend/api/routes/sessions.py`
  - Endpoint: POST /sessions → SessionResponse
  - Acceptance: T097 passes

- [x] T099 [RED] [P] Write test for GET /sessions/{id}
  - File: `tests/integration/test_sessions.py`
  - Test: Returns session details, 404 if not found
  - FR: FR-012

- [x] T100 [GREEN] [P] Implement GET /sessions/{id} endpoint
  - File: `backend/api/routes/sessions.py`
  - Endpoint: GET /sessions/{id} → SessionDetailResponse
  - Acceptance: T099 passes

- [x] T101 [RED] [P] Write test for DELETE /sessions/{id}
  - File: `tests/integration/test_sessions.py`
  - Test: Marks session inactive, returns 204
  - FR: FR-012

- [x] T102 [GREEN] [P] Implement DELETE /sessions/{id} endpoint
  - File: `backend/api/routes/sessions.py`
...
- [x] T103 [RED] [B] Write test for GET /sessions/{id}/messages
  - File: `tests/integration/test_sessions.py`
  - Test: Returns messages with pagination, excludes expired
  - FR: FR-027

- [x] T104 [GREEN] [B] Implement GET /sessions/{id}/messages endpoint
  - File: `backend/api/routes/sessions.py`
...
- [x] T105 [RED] [B] Write test for message TTL calculation
  - File: `tests/unit/test_database_models.py`
  - Test: expires_at = created_at + 24h
  - FR: FR-027

- [x] T106 [GREEN] [B] Implement message TTL in ChatMessage model
  - File: `backend/models/database.py`
  - Modification: Set expires_at on creation
  - Acceptance: T105 passes

- [x] T107 [RED] [P] Write test for expired message filtering
  - File: `tests/unit/test_database.py`
  - Test: Query excludes messages where expires_at < now
  - FR: FR-027

- [x] T108 [GREEN] [P] Implement expired message filter
  - File: `backend/core/database.py`
...
- [x] T109 [B] Create Qdrant collection setup script
...
## Phase 11: Frontend Widget (Complete)

> React chat widget for Docusaurus integration.
> Dependencies: All backend phases

- [x] T112 [P] Create ChatWidget React component
  - Files: `src/components/ChatWidget/index.tsx`, `useChat.ts`, `types.ts`, `styles.module.css`
  - Features: Message list, input, WebSocket connection, persona selector, citations
  - FR: FR-001, FR-002, FR-003, FR-012, NFR-001

- [x] T113 [P] Create SelectionTrigger component
  - Files: `src/components/SelectionTrigger/index.tsx`, `styles.module.css`
  - Features: Text selection detection, popover tooltip, custom event dispatch
  - FR: FR-018, FR-019

- [x] T114 [P] Implement accessibility (WCAG 2.1 AA)
  - Files: `src/theme/Root.tsx`, `src/components/ChatWidget/styles.module.css`, `src/components/SelectionTrigger/styles.module.css`
  - Features: ARIA labels, keyboard nav (Escape/Enter), focus management, reduced motion, screen reader announcements
  - FR: NFR-002 (WCAG 2.1 AA), FR-002
...
- [x] T115 [RED] [B] Write test for message purge job
  - File: `tests/unit/test_purge.py`
  - Test: Deletes messages where expires_at < now
  - FR: FR-027

- [x] T116 [GREEN] [B] Implement message purge background task
...
- [x] T119 [P] Add structured logging throughout
  - Files: Various backend files
  - Format: JSON with request_id, session_id, latency
  - FR: N/A (operations)

- [x] T120 [P] Add request timing middleware
  - File: `backend/api/middleware.py`
...
(US2:Persona)  (US5:Safety)   (US3:Page)    (US4:Sel) │
    ...
    └──────────────┴──────────────┴──────────────┘    │
                   ...
                   Phase 12 (Operations)
                        │
                        ▼
                   Phase 13 (Page Content Context)
                   [US3 Enhancement - FR-031-034]
```

---

## Phase 13: Page Content Context (US3 Enhancement)

> Enable answering vague contextual queries like "explain this page" by sending page content to backend.
> Dependencies: Phase 6 (Page-scoped), Phase 11 (Frontend)
> FR References: FR-031, FR-032, FR-033, FR-034

### 13.1 Backend: Vague Query Detection

- [x] T121 [RED] [B] [US3] Write test for vague contextual query detection in `tests/unit/test_vague_query_detection.py`
  - Test: `is_vague_contextual_query("explain this page")` returns `True`
  - Test: `is_vague_contextual_query("what is this about")` returns `True`
  - Test: `is_vague_contextual_query("summarize this")` returns `True`
  - Test: `is_vague_contextual_query("what is ROS 2?")` returns `False`
  - Test: `is_vague_contextual_query("how do nodes work?")` returns `False`
  - FR: FR-033

- [x] T122 [GREEN] [B] [US3] Implement vague query detection helper in `backend/services/rag.py`
  - Method: `is_vague_contextual_query(query: str) -> bool`
  - Pattern matching for: "explain this", "what is this", "summarize", "tell me about this page", etc.
  - Use keyword-based detection (not ML classifier per plan.md decision)
  - Acceptance: T121 passes

### 13.2 Backend: Accept Page Content in WebSocket

- [x] T123 [RED] [B] [US3] Write test for page_content in WebSocket context message in `tests/integration/test_page_content_context.py`
  - Test: WebSocket accepts `{"type": "context", "data": {"page_content": "..."}}`
  - Test: Page content stored in session state (memory)
  - Test: Page content limited to 8000 chars (truncated if longer)
  - FR: FR-032, FR-034

- [x] T124 [GREEN] [B] [US3] Update WebSocket handler to accept page_content in `backend/api/routes/chat.py`
  - Add `page_content` field to context message handling
  - Store in `current_page_context` dict (memory, not DB)
  - Truncate to 8000 chars per FR-034
  - Acceptance: T123 passes

### 13.3 Backend: Inject Page Content into RAG Context

- [x] T125 [RED] [B] [US3] Write test for page content injection in RAG in `tests/unit/test_page_content_injection.py`
  - Test: Vague query + page content available → page content prepended to RAG context
  - Test: Vague query + no page content → falls back to normal RAG
  - Test: Specific query + page content available → uses normal RAG (ignores page content)
  - Test: Context format: `[Current Page Content]\n{page_content}\n\n[RAG Results]\n{rag_context}`
  - FR: FR-033

- [x] T126 [GREEN] [B] [US3] Implement page content injection in `backend/api/routes/chat.py`
  - In WebSocket handler, detect vague query using T122 helper
  - When vague AND page_content available: prepend to RAG context
  - Update RAGPipeline call to include page content parameter
  - Acceptance: T125 passes

### 13.4 Frontend: Extract Page Content

- [x] T127 [P] [US3] Implement page content extraction in `src/theme/Root.tsx`
  - Added `extractPageContent(): string` helper function
  - Extracts text from `<article>` element (Docusaurus main content area)
  - Strips HTML tags, normalizes whitespace
  - Truncates to 8000 chars
  - FR: FR-031, FR-034

### 13.5 Frontend: Send Page Content on Connect/Navigation

- [x] T128 [P] [US3] Update Root.tsx to pass page content to ChatWidget in `src/theme/Root.tsx`
  - Extracts page content on page navigation (route change)
  - Uses useEffect with location.pathname dependency
  - Passes pageContent as prop to ChatWidget

- [x] T129 [P] [US3] Update useChat to send page_content in context message in `src/components/ChatWidget/useChat.ts`
  - Added `pageContent` to UseChatOptions and ChatWidgetProps
  - Includes `page_content` field in WebSocket context message
  - Sends on connect and on page change
  - FR: FR-031
...
### Phase 13 (Page Content Context):
```bash
# Backend tasks run sequentially (T121-T126):
cd backend && source .venv/bin/activate
PYTHONPATH=. python -m pytest tests/unit/test_vague_query_detection.py -v
PYTHONPATH=. python -m pytest tests/integration/test_page_content_context.py -v
PYTHONPATH=. python -m pytest tests/unit/test_page_content_injection.py -v

# Frontend tasks can run in parallel (T127-T129):
...
| 13 | T121-T131 | P2 | US3 (Page Content Context) |

**Total Tasks**: 146
**TDD Tasks (RED+GREEN pairs)**: 118 (59 test pairs)
**Blocking Tasks**: 73
**Parallelizable Tasks**: 73
